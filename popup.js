        ...
        func: (sel, count) => {
            ...
            function replaceTextNodes(node) {
                if (node.nodeType === Node.TEXT_NODE) {
                    ...
                    node.textContent = words.sort(() => 0.5 - Math.random()).slice(0, count).join(' ');
            ...
            console.log(`Texte remplacé dans ${elements.length} élément(s) avec ${count} caractères.`);
        },
        args: [selector, charCount]
    });
});
